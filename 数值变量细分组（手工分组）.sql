...
/*create table  sco.zs_CAR_var_pre_v1
(
...
from
(
select case when t.GPS_SUM<=1068  then '(-∞,1068]' 
            when t.GPS_SUM> 1068 then '(1068,+∞]' 
                 else 'other' end  group_name     
       ,count(*) N 
       ,sum(t.target) N1
from sco.zs_car_basetrain t 
group by cube(case when t.GPS_SUM<=1068  then '(-∞,1068]' 
                   when t.GPS_SUM> 1068 then '(1068,+∞]' 
                   else 'other' end)
);
commit;
...
---7、PUTOUT_APPLYAMOUNT 放款申请次数
insert into  sco.zs_CAR_var_pre_v1(state_name,var_name,group_type,group_name,N,N1,state_Interva_sdate,state_Interva_edate,create_time)
select '宽表变量分析20160517' state_name,
       upper('PUTOUT_APPLYAMOUNT ') var_name,
       decode(group_name,null,'PUTOUT_APPLYAMOUNT '||'分析',null) group_type,
       group_name,
       decode(group_name,null,null,N) N,
       decode(group_name,null,null,N1) N1,
       to_date('20140827','yyyymmdd') state_Interva_sdate,
...
from
(
select case when PUTOUT_APPLYAMOUNT=1  then 'app_1' 
            when PUTOUT_APPLYAMOUNT=2  then 'app_2'  
            when PUTOUT_APPLYAMOUNT>=3 then 'app_3'                                          
            else 'other' end  group_name     
       ,count(*) N 
       ,sum(t.target) N1
from cu.zs_car_add_basetrain t 
group by cube(case when PUTOUT_APPLYAMOUNT=1 then 'app_1' 
                   when PUTOUT_APPLYAMOUNT=2 then 'app_2'  
                   when PUTOUT_APPLYAMOUNT>=3 then 'app_3'                                          
                   else 'other' end)
);
...
from
(
select case when MONTH_TOTAL<=6300  then '(-∞,6300]' 
            when MONTH_TOTAL<=22500 then '(6300,22500]'  
            when MONTH_TOTAL>22500 then '(22500,+∞]'                                          
            else 'other' end  group_name     
       ,count(*) N 
       ,sum(t.target) N1
from cu.zs_car_add_basetrain t 
group by cube(case when MONTH_TOTAL<=6300  then '(-∞,6300]' 
            when MONTH_TOTAL<=22500 then '(6300,22500]'  
            when MONTH_TOTAL>22500 then '(22500,+∞]'                                          
            else 'other' end)
);
