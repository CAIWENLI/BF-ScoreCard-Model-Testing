-----------------------------车贷宽表--------------------------------
grant select ,insert,delete on cu.zs_car_base_total2 to zhangshuang,sco,tangjiajie;
create table cu.zs_car_base_total2
as
(
SELECT 
CONTRACT_NO
...
,SERVICEPROVIDERSACCOUNT_TYPE
,SERVICEPROVIDERS_ACCOUNTT
,SERVICEPROVIDERS_BRANCH
,WITNESS_CERTNO
,WITNESS_NAME
,WITNESS_PHONE
,PRODUCT_NAME
...
,PARTICIPANT_ADD
,WORKBEGINDATE
,(case when a.product_type in ('车抵融','快抵融','第一车网','一证通（二手车）','二手宝捷融','二手快捷融','二手轻松购','二手融e购')  then '二手车'
when a.product_type in ('宝捷融LCV','聚宝融','快捷融','快捷融2.0','气球融','轻卡融','舒心融','新车宝捷融') then '新车'
...
select  
   ...
   case when bc.city<>'150000' then 
   (select rcas.pkg_utl.f_get_province_name(a.itemno)  from  s3.code_library a where a.codeno ='AreaCode'  and a.itemno=bc.city) 
     else '内蒙古自治区' end  province,
